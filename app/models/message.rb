class Message < ApplicationRecord
  ...
  def ensure_conversation
    self.conversation ||= Conversation.find_by(sender_id: self.user_id, listing_id: listing_id)

    if self.conversation.nil? && listing_id
      recipient_id = Listing.find(listing_id).user_id
      self.conversation = Conversation.new(sender_id: self.user_id, listing_id: listing_id, recipient_id: recipient_id, offer: offer)
